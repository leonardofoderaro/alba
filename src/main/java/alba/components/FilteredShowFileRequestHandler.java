...
public class FilteredShowFileRequestHandler extends RequestHandlerBase
{
	...
	protected static final Logger log = LoggerFactory
			.getLogger(FilteredShowFileRequestHandler.class);
	...
	private void showFromZooKeeper(SolrQueryRequest req, SolrQueryResponse rsp,
		...
		if (children.size() > 0) {
			...
			content = new ContentStreamBase.ByteArrayStream(sw.getBuffer().toString().getBytes(Charset.forName("UTF-8")), adminFile);
			content.setContentType(req.getParams().get(USE_CONTENT_TYPE));
